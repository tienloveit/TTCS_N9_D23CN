...
public class BookingService {
    ...
    public BookingResponse createBooking(CreateBookingRequest request) {
        ...
        boolean hasUnavailableTicket = selectedTickets.stream()
                .anyMatch(
                        ticket -> ticket.getTicketStatus() != TicketStatus.AVAILABLE
        ...
        for (Ticket ticket : selectedTickets) {
            ...
            Boolean locked = redisTemplate.opsForValue().setIfAbsent(seatKey, bookingUserId, holdMinutes,
                    TimeUnit.MINUTES);
        ...
        BigDecimal totalAmount = selectedTickets.stream().map(Ticket::getPrice).reduce(BigDecimal.ZERO,
                BigDecimal::add);
    ...
    public BookingResponse createStaffBooking(StaffBookingRequest request) {
        ...
        foodInventoryService.requireAvailable(selectedFoods, foodQuantities);
        BigDecimal foodTotal = calculateFoodTotal(selectedFoods, foodQuantities);
        BigDecimal ticketTotal = selectedTickets.stream().map(Ticket::getPrice).reduce(BigDecimal.ZERO,
                BigDecimal::add);
        ...
        Booking booking = Booking.builder()
                ...
                .promotionCode(appliedPromoCode)
                .discountAmount(discountAmount)
                .status(request.getPaymentMethod() == PaymentMethod.CARD ? BookingStatus.PENDING
                        : BookingStatus.COMPLETED)
                .paymentCreatedAt(now)
                .paymentStatus(
                        request.getPaymentMethod() == PaymentMethod.CARD ? PaymentStatus.PENDING : PaymentStatus.PAID)
        ...
        selectedTickets.forEach(
                ticket -> {
                    ...
                    ticket.setTicketStatus(
                            request.getPaymentMethod() == PaymentMethod.CARD ? TicketStatus.HOLDING
                                    : TicketStatus.BOOKED);
                    if (request.getPaymentMethod() != PaymentMethod.CARD) {
    ...
    public List<BookingResponse> getRefundBookings() {
        return scopeBookingsForBranchOperator(bookingRepository.findAll()).stream()
                .filter(
                        booking -> booking.getStatus() == BookingStatus.REFUND_REQUESTED
    ...
    public BookingResponse getBookingById(Long bookingId) {
        ...
        boolean isBranchManager = isBranchOperator(user)
    ...
    public void cancelBooking(Long bookingId) {
        ...
        User user = getUserCurrent();
        boolean isOwner = booking.getUser().getId().equals(user.getId());
        boolean canManageBooking = user.getRole() == UserRole.ADMIN
    ...
    private void unlockSeats(Long showTimeId, List<Ticket> tickets) {
        try {
            List<String> keys = tickets.stream().map(ticket -> getSeatKey(showTimeId, ticket.getSeat().getId()))
                    .toList();
    ...
    private String getOrCreateTicketQrCode(Booking booking, Ticket ticket) {
        ...
        String seatCode = ticket.getSeat() == null || ticket.getSeat().getSeatCode() == null
                ? ""
    ...
    private List<Food> getSelectedFoods(Map<Long, Integer> foodQuantities) {
        ...
        boolean hasUnavailableFood = selectedFoods.size() != foodQuantities.size()
    ...
    private void requireFoodsAvailableForShowtime(List<Food> foods, Showtime showtime) {
        ...
        Long branchId = showtime.getRoom() == null || showtime.getRoom().getBranch() == null
                ? null
                : showtime.getRoom().getBranch().getBranchId();
        boolean hasFoodFromOtherBranch = foods.stream()
                .anyMatch(
     ...
     * Chỉ cho phép trong vòng 24 giờ sau khi thanh toán, booking phải ở trạng thái
     * COMPLETED.
     */
    @Transactional
    ...
    public BookingResponse processRefund(Long bookingId, boolean approved, String note) {
        ...
        if (approved) {
            ...
            if (user != null && user.getTotalSpending() != null) {
                ...
                if (newSpending.compareTo(BigDecimal.ZERO) < 0)
                    newSpending = BigDecimal.ZERO;
