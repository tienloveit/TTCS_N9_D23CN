...
public class PromotionService {
    ...
    public List<PromotionResponse> getAvailablePromotions() {
        User user = getUserCurrent();
        MembershipTier userTier = user.getMembershipTier() != null ? user.getMembershipTier() : MembershipTier.BRONZE;
                ...
                .filter(
                        p -> {
                            ...
                            if (p.getMinMembershipTier() != null
                                    && userTier.ordinal() < p.getMinMembershipTier().ordinal())
                                return false;
                            return true;
                        })
                .map(this::toResponse)
    ...
    /**
     * Validate mã giảm giá và trả về số tiền được giảm. Nếu không hợp lệ, throw
     * exception.
     */
    @Transactional(readOnly = true)
    public BigDecimal validatePromotion(
        ...
        if (promotion.getMinMembershipTier() != null) {
            ...
            MembershipTier userTier = user.getMembershipTier() != null ? user.getMembershipTier()
