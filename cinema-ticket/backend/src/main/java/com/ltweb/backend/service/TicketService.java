...
public class TicketService {
    ...
    public void createTicket(Showtime showtime) {
        ...
        // lấy sẵn seatTypePrice ra thay vì mỗi lần lại phải chọc vào db để lấy, tránh N
        // + 1
    ...
    private TicketCheckInResponse buildCheckInResponse(
        ...
        return TicketCheckInResponse.builder()
                ...
                .tickets(
                        tickets.stream()
                                .map(ticket -> toCheckInItem(ticket,
                                        alreadyCheckedIn.getOrDefault(ticket.getId(), false)))
                                .toList())
